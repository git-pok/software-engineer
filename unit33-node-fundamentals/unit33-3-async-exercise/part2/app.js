...
async function shuffleDeck(deckBaseUrl) {
    const res = await axios.get(`${deckBaseUrl}`);
    return res
}

async function drawCard(deckId) {
    const cardDrawUrl = `https://deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`;
    return await axios.get(`${cardDrawUrl}`);
}
...
async function drawCardGame() {
    if (!deckDrawId) {
        let deck = await shuffleDeck(deckBaseUrl);
        const deckId = deck.data.deck_id;    
        deckDrawId = deckId;
        let cardDrawRes = await drawCard(deckId);        
        const cards = cardDrawRes.data.cards;
        obj = logCard(cards);
        createCardHtml(obj);
    } else {
        let cardDrawRes = await drawCard(deckDrawId);
        const cards = cardDrawRes.data.cards;
        obj = logCard(cards);
        createCardHtml(obj);
    }       
