...
and the values as the number of times the vowel
...
function vowelCount(str) {
    const vowels = 'aeiou';
    let lwrCaseStr = str.toLowerCase().split('');
    const vowelObj = lwrCaseStr.reduce((acc, next) => {
        const isVowel = vowels.includes(next);
        if (isVowel) {
...
passed to it with each object now including the key and
...
array and a callback and returns an array with two
arrays inside of it. The partition function should run
the callback function on each value in the array and
if the result of the callback function at that specific
value is true, the value should be placed in the first subarray.
If the result of the callback function at that specific value
...
function partition(arr, callback) {
    return arr.reduce((acc, next) => {
        const isCallbackTrue = callback(next);
        if (isCallbackTrue) acc[0].push(next);
